...
func main() {
	...
	app.Action = func() {
		...
		sectionsDriver := sections.NewCypherSectionsService(db)
		sectionsDriver.Initialise()
		...
		baseftrwapp.RunServer(endpoints,
			v1a.Handler("ft-sections_rw_neo4j ServiceModule", "Writes 'sections' to Neo4j, usually as part of a bulk upload done on a schedule", checks...),
			*port, "sections-rw-neo4j", "local")
	}
...
func makeCheck(service baseftrwapp.Service, cr neoutils.CypherRunner) v1a.Check {
	return v1a.Check{
		BusinessImpact:   "Cannot read/write sections via this writer",
